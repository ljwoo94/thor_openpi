...
def run_flashrt_inference(
    ...
    """Run FlashRT inference with warmup and multiple test runs."""
    print("\n--- FlashRT Inference ---")
    print("Loading FlashRT policy...")
    policy = policy_config.create_flashrt_policy(
        config,
        checkpoint_dir,
        framework=framework,
        hardware=hardware,
    ...
    print("FlashRT policy loaded successfully")
...
def compare_outputs(reference_actions, candidate_actions, candidate_name="TensorRT"):
    """Compare reference and candidate policy outputs."""
    print("\n" + "=" * 60)
    print("Comparison Results:")
    print("=" * 60)

    # Shape comparison
    print(f"PyTorch shape:  {reference_actions.shape}")
    print(f"{candidate_name} shape: {candidate_actions.shape}")
    ...
    # Cosine Similarity
    pytorch_flat = reference_actions.flatten()
    candidate_flat = candidate_actions.flatten()

    dot_product = np.dot(pytorch_flat, candidate_flat)
    pytorch_norm = np.linalg.norm(pytorch_flat)
    candidate_norm = np.linalg.norm(candidate_flat)
    cosine_similarity = dot_product / (pytorch_norm * candidate_norm + 1e-8)

    print("\nCosine Similarity:")
    print(f"  - Overall: {cosine_similarity:.8f}")

    # Per-timestep cosine similarity
    if len(reference_actions.shape) >= 2:
        timestep_similarities = []
        for t in range(reference_actions.shape[0]):
            pt_vec = reference_actions[t].flatten()
            candidate_vec = candidate_actions[t].flatten()
            dot = np.dot(pt_vec, candidate_vec)
            norm_pt = np.linalg.norm(pt_vec)
            norm_candidate = np.linalg.norm(candidate_vec)
            sim = dot / (norm_pt * norm_candidate + 1e-8)
    ...
    diff = np.abs(reference_actions - candidate_actions)
    ...
    # Relative error
    rel_error = diff / (np.abs(reference_actions) + 1e-6)
    print("\nRelative Error:")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="pytorch",
        choices=["pytorch", "tensorrt", "flashrt", "compare", "compare-flashrt"],
        help="Inference mode: pytorch, tensorrt, flashrt, compare, or compare-flashrt (default: pytorch)",
    )
    parser.add_argument(
        "--config-name",
        type=str,
    ...
    parser.add_argument(
        ...
        default="torch",
        choices=["torch", "jax"],
        help="FlashRT framework backend (default: torch)",
    )
    parser.add_argument(
        "--flashrt-hardware",
        type=str,
        ...
        help="FlashRT hardware backend (default: thor)",
    )
    parser.add_argument(
        "--flashrt-num-views",
        type=int,
        default=2,
        help="FlashRT camera view count for LIBERO (default: 2)",
    )
    parser.add_argument(
        "--flashrt-autotune",
        type=int,
        default=3,
        help="FlashRT CUDA graph autotune trials (default: 3)",
    )
    parser.add_argument(
        "--flashrt-use-fp4",
        action="store_true",
        help="Enable FlashRT NVFP4 path when supported by hardware",
    )
    parser.add_argument(
        "--flashrt-recalibrate",
        action="store_true",
        help="Force fresh FlashRT calibration",
    )
    ...
    if args.inference_mode == "compare":
        ...
        # Compare outputs
        compare_outputs(pytorch_actions, tensorrt_actions, candidate_name="TensorRT")
    ...
    elif args.inference_mode == "compare-flashrt":
        print("\n[1/3] Loading example...")
        example = load_example(config, args.use_dataset, args.sample_idx)

        print("\n[2/3] Running both PyTorch and FlashRT inference...")
        flashrt_actions, flashrt_inference_stats, flashrt_model_stats = run_flashrt_inference(
            config,
            checkpoint_dir,
            example,
            num_warmup=args.num_warmup,
            num_test_runs=args.num_test_runs,
            framework=args.flashrt_framework,
            hardware=args.flashrt_hardware,
            num_views=args.flashrt_num_views,
            autotune=args.flashrt_autotune,
            use_fp4=args.flashrt_use_fp4,
            recalibrate=args.flashrt_recalibrate,
        )
        pytorch_actions, pytorch_inference_stats, pytorch_model_stats = run_pytorch_inference(
            config,
            checkpoint_dir,
        ...
        print("\nFlashRT:")
        print(f"  - Actions range: [{flashrt_actions.min():.4f}, {flashrt_actions.max():.4f}]")
        print(f"  - Total time: {flashrt_inference_stats['mean']:.2f} ± {flashrt_inference_stats['std']:.2f} ms")
        print(f"    (min: {flashrt_inference_stats['min']:.2f}, max: {flashrt_inference_stats['max']:.2f})")
        print(f"  - Model time: {flashrt_model_stats['mean']:.2f} ± {flashrt_model_stats['std']:.2f} ms")
        print(f"    (min: {flashrt_model_stats['min']:.2f}, max: {flashrt_model_stats['max']:.2f})")

        print("\nSpeedup:")
        speedup_total = pytorch_inference_stats["mean"] / flashrt_inference_stats["mean"]
        speedup_model = pytorch_model_stats["mean"] / flashrt_model_stats["mean"]
        print(f"  - Total: {speedup_total:.2f}x")
        print(f"  - Model: {speedup_model:.2f}x")

        compare_outputs(pytorch_actions, flashrt_actions, candidate_name="FlashRT")
        print("\nNOTE: FlashRT currently samples diffusion noise internally; compare-flashrt is not a fixed-noise test.")
    ...
    elif args.inference_mode == "flashrt":
        print("\n[1/2] Loading example...")
        example = load_example(config, args.use_dataset, args.sample_idx)

        print("\n[2/2] Running FlashRT inference...")
        action_chunk, inference_stats, model_stats = run_flashrt_inference(
            config,
            checkpoint_dir,
            example,
            num_warmup=args.num_warmup,
