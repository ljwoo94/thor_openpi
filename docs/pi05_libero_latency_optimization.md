...
| 2026-05-14 | `666ec8e` | Add denoising-step sweep support to the benchmark and make Thor ONNX export use the configured PyTorch image-key subset. | Lets the user measure total denoising-cost scaling and lets TensorRT export avoid the unused LIBERO camera input. | `py_compile` and `git diff --check` passed locally. `uv run ... --help` is blocked on Apple Silicon by the pinned `jax-cuda12-plugin`; H100/Jetson benchmark and Thor export required. | Pending. |
