...
| 2026-05-14 | pending | Revert batched vision prefix while keeping QDQ coverage, and add static-shape ONNX export option. | Avoids compile-breaking prefix logic and gives TensorRT a fixed-shape export path for stronger engine optimization. | `py_compile` and `git diff --check` passed locally. Thor static-shape export and engine build required. | Requested after compile conversion failure. |
