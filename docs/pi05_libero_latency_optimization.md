...
| 2026-05-13 | pending | Precompute pi05 action suffix masks, position offsets, and timestep embedding basis. | Reduces small tensor construction inside every denoising step and makes the suffix path more static for later compile work. | `py_compile` and `git diff --check` passed locally. NVIDIA benchmark required for latency and correctness. | Pending. |
| 2026-05-13 | pending | Move fixed attention backend setup out of the inference hot path. | Removes repeated config mutation from prefix encode and every denoise step, reducing Python side effects before compile-region work. | `py_compile` and `git diff --check` passed locally. NVIDIA benchmark required for latency and correctness. | Pending. |
