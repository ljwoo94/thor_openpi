...
| 2026-05-14 | pending | Revert low-impact prompt token cache and suffix tensor precomputation. | Keeps optimization work focused on core VLM forward, denoise loop, attention kernels, quantization coverage, and TensorRT graph conversion. | Local checks pending. | Requested after user validation. |
