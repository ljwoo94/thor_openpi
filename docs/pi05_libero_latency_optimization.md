...
| 2026-05-14 | `5acb667` | Add ModelOpt/ONNX QDQ coverage reports and batch all camera images through one vision-tower forward. | Makes FP8 coverage gaps visible and targets the VLM prefix forward directly by replacing three vision forwards with one larger batched forward. | `py_compile` and `git diff --check` passed locally. H100/Jetson export plus fixed-noise latency/correctness benchmark required. | Pending. |
...
| 2026-05-14 | pending | Make Thor ONNX export denoise loop static and remove export hot-path attention config mutation. | Improves TensorRT/ONNX graph conversion for the real repeated denoise forward instead of Python-side preprocessing. | `py_compile` and `git diff --check` passed locally. Thor export and TensorRT build required. | Pending. |
